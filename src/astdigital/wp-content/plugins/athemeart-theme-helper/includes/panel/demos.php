    ...
    class aThemeArt_Demos {
        ...
        public static function scripts($hook_suffix) {
            ...
            if ('appearance_page_athemeart-panel-install-demos' == $hook_suffix) {
                ...
                // JS
                wp_enqueue_script('athemeart-demos-js', plugins_url('/assets/js/demos.js', __FILE__), array('jquery', 'wp-util', 'updates'), '1.0', true);
        ...
        public static function required_plugins( $plugins, $return ) {
            foreach ( $plugins as $plugin ) {
                ...
                if ( file_exists( WP_PLUGIN_DIR . '/' . $api['init'] ) && is_plugin_inactive( $api['init'] ) ) {
                    $button_classes = 'button activate-now button-primary';
                    $button_text    = esc_html__( 'Activate', 'athemeart-theme-helper' );
                // Not Installed
                } elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $api['init'] ) ) {
                ...
                $button_classes = !empty($api['source']) ? $button_classes . ' premium' : $button_classes;
                ?>
                <div class="athemeart-plugin athemeart-clr athemeart-plugin-<?php echo esc_attr( $api['slug'] ); ?>"
                     data-slug="<?php echo esc_attr( $api['slug'] ); ?>"
                     data-init="<?php echo esc_attr( $api['init'] ); ?>"
                     data-source="<?php echo esc_attr( $api['source'] ); ?>">
        ...
        public function ajax_required_plugins_activate() {
            ...
            if ( ! current_user_can( 'install_plugins' ) ) {
                wp_send_json_error( array(
                    'success' => false,
                    'message' => __( 'You do not have permission to install plugins.', 'athemeart-theme-helper' ),
                ) );
            }
            ...
            if ( file_exists( WP_PLUGIN_DIR . '/' . $init ) ) {
                $this->do_activate_plugin( $init );
                return;
            }
            ...
            // Silent skin so we get clean JSON
        ...
        private function do_activate_plugin( $init ) {
            $activate = activate_plugin( $init, '', false, true );
