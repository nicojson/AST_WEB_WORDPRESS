...
add_action( 'admin_notices', 'athemeart_demo_import_notice' );

function athemeart_demo_import_notice() {

   
    // Only users who can manage plugins.
    if ( ! current_user_can( 'manage_options' ) ) {
        return;
    }

    // Already dismissed?
    if ( get_user_meta( get_current_user_id(), 'athemeart_demo_import_notice_dismissed', true ) ) {
        return;
    }
    $theme = wp_get_theme();
    ?>
    <div class="notice notice-success is-dismissible athemeart-demo-import-notice">
        <p>
            <strong>
                <?php
                printf(
                    /* translators: %s: Theme name */
                    esc_html__( 'Thanks for installing %s Demo Import!', 'athemeart-theme-helper' ),
                    esc_html( $theme['Name'] )
                );
                ?>
            </strong>
        ...
        <p>
            ...
            printf(
                ...
                esc_html__( 'Import your demo content in one click from Appearance → %1$s, or %2$sclick here%3$s.', 'athemeart-theme-helper' ),
                esc_html( $theme['Name'] ),
                '<a href="' . esc_url( admin_url( 'themes.php?page=athemeart-panel-install-demos' ) ) . '">',
                '</a>'
            );
            ?>
        </p>
...
add_action( 'admin_enqueue_scripts', 'athemeart_demo_import_notice_script' );

function athemeart_demo_import_notice_script() {

    wp_add_inline_script(
        'common',
        "
        jQuery(function($){
            $(document).on('click', '.athemeart-demo-import-notice .notice-dismiss', function () {
                $.post(ajaxurl, {
                    action: 'athemeart_demo_import_dismiss_notice',
                    nonce: '" . wp_create_nonce( 'athemeart_demo_import_notice' ) . "'
                });
            });
        });
        "
...
add_action( 'wp_ajax_athemeart_demo_import_dismiss_notice', 'athemeart_demo_import_dismiss_notice' );

function athemeart_demo_import_dismiss_notice() {

    check_ajax_referer( 'athemeart_demo_import_notice', 'nonce' );

    update_user_meta(
        get_current_user_id(),
        'athemeart_demo_import_notice_dismissed',
        1
    );
